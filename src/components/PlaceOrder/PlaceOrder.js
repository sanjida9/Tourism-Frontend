...
const PlaceOrder = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5001/spot/${id}`)
  ...
  const onSubmit = data => {
    ...
    axios.post("http://localhost:5001/placeOrder", data)
      .then(res => {
  ...
  return (
    <div className="container row my-5">
      ...
      <form className="col-md-6 mx-auto" onSubmit={handleSubmit(onSubmit)} >
        <input className="form-control" {...register("fullName", { required: true })} placeholder="Full Name" defaultValue={user.displayName} />
        {errors.fullName && <span>This field is required</span>}
        <br /><br />
        <input className="form-control" {...register("email", { required: true })} placeholder="Email" defaultValue={user.email} />
        {errors.email && <span>This field is required</span>}
        <br /><br />
        <input className="form-control" {...register("address", { required: true })} placeholder="Address" />
        {errors.address && <span>This field is required</span>}
        <br /><br />
        <input defaultValue="1" min="1" onChangeCapture={handleQuantity} type="number" className="form-control" {...register("quantity", { required: true })} placeholder="Quantity" />
        ...
        <br /><br />
        <input type="submit" value="Place Your Order" className="btn btn-success" />
