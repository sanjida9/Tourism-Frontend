...
const ManageAllOrders = () => {
  ...
  const handleDeleteOrder = id => {
    ...
    if (isDelete) {
      axios.post("http://localhost:5001/deleteOrder", { "UserId": id })
        .then(res => {
  ...
  const handleUpdateStatus = (e) => {
    ...
    axios.post('http://localhost:5001/updateStatus', { "status": status, "id": id })
      .then(res => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5001/manageAllOrders")
        ...
        <tbody>
          ...
          {allOrders.map(order =>
            <tr>
              ...
              <td>
                <form className="d-flex align-items-center me-4" onSubmit={e => handleUpdateStatus(e)}>
                  < select defaultValue={order.status} id={order._id}

                    className="border-2 border-gray-100 form-select block w-full p-2" >
                    <option value="Pending">Pending</option>
                    <option value="Approved">Approved</option>
                  </select >
                  <button type="submit" className="btn btn-warning" type="submit"><i className="fas fa-check"></i></button>
                </form>
